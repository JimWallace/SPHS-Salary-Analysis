...
\textbf{Step 1 --- Unconditional means model.} Fitting salary with random intercepts only, the model partitions variance into between-faculty (\(\hat{\tau}^2_{00} = 340{,}980{,}617\)) and within-faculty components (\(\hat{\sigma}^2 = 791{,}036{,}685\)), yielding an ICC of \(0.301\). This indicates that 30.1\% of total salary variance reflects persistent differences between faculty members, justifying a multilevel approach and establishing variance structure before any predictors are added.

\textbf{Step 2 --- Unconditional growth model.} Adding time as a fixed predictor with faculty-specific random slopes dramatically improves model fit: LRT \(\chi^2(3) = 962.85\), \(p < 0.001\). The estimated average annual salary increase across all faculty is \(\$7{,}763\)/year. Substantial between-faculty variation in slopes (\(\hat{\tau}^2_{\text{slope}} = 2{,}448{,}000\)) confirms that individual growth trajectories differ considerably, which must be modelled before attributing group differences.

\textbf{Step 3 --- Conditional growth model.} Adding MHI affiliation and its interaction with time further improves fit: LRT \(\chi^2(2) = 11.88\), \(p = 0.003\). The MHI main effect estimates a level gap of approximately \(-\$34{,}477\) at the mean year (\(p = 0.009\)), indicating that MHI-affiliated faculty have lower average salaries even before accounting for differences in growth. The primary growth-gap estimate --- the \texttt{time:mhi} interaction --- is \(-\$1{,}595\)/year (95\% CI: \([-\$3{,}121,\ -\$69]\), \(p = 0.049\)). Non-MHI faculty grow at approximately \(\$8{,}077\)/year, while MHI faculty grow at roughly \(\$6{,}482\)/year, a difference of approximately 20\% per year.
